...
from .. import _keys
from nequip.train._loss import torch_corr, ccc_loss
...
class EdgewiseEnergySum(GraphModuleMixin, torch.nn.Module):
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        edge_feature = data["edge_features"]
        env_features = data["features"]
        # over-smoothing
        # data["sim"] = torch.tensor(get_pairwise_sim(edge_feature))
        # over-correlation
        corr = torch_corr(edge_feature[:1000].t())
        # corr = ccc_loss(edge_feature[:1000].t())
        data["corr"] = corr
