...
class ScalarMLPFunction(CodeGenMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        self.mlp_input_dimension = mlp_input_dimension
        ...
        self.mlp_initialization = mlp_initialization
        ...
        features = Proxy(self.graph.placeholder("x"))
        ...
        for layer, (h_in, h_out) in enumerate(zip(dimensions, dimensions[1:])):
            ...
            if mlp_dropout_p > 0:
                ...
                features = Proxy(self.graph.call_module("_dropout", (features.node,)))
            ...
            w = Proxy(self.graph.get_attr(f"_weight_{layer}"))
            w = w * (
            ...
            if layer == 0:
                self.para = norm_from_last / math.sqrt(float(h_in))
            if mlp_batchnorm:
                # if we call batchnorm, do it after the nonlinearity
                features = Proxy(self.graph.call_module(f"_bn_{layer}", (features.node,)))
        ...
        self.graph.output(features.node)
        ...
        self._codegen_register({"_forward": fx.GraphModule(base, self.graph)})
