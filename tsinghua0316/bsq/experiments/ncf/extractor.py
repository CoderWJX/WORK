...
class MyExtractor(object):
    ...
    def myhook(self, module, input, output):
        if isinstance(module, QuantizedLinear):
            if module.is_quan_a:
                need = ActQuanFunc.apply(input[0].detach(), module.a_s, module.a_low, module.a_high)/module.a_s
            else:
                if 'MLP_layers.1' in self.module2name[module]:
                    need = input[0].detach()/module.a_s
                else:
                    need = input[0].detach()
        else:
            need = input[0].detach()
        self.gpu_dict[str(input[0].device)][self.module2name[module]] = need
