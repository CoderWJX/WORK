...
def save_checkpoint(epoch, arch, model, extras=None, is_best=None, name=None, output_dir='.'):
    ...
    if not os.path.isdir(output_dir):
        raise IOError('Checkpoint directory does not exist at',
                      os.path.abspath(dir))
    ...
    filename_best_pth = 'best.pth' if name is None else name + '_best.pth'
    filepath_best_pth = os.path.join(output_dir, filename_best_pth)
    ...
    torch.save(model_dict, os.path.join(output_dir, 'model.pth' if name is None else name + '.pth'))
    if is_best:
        msg += '                Best: %s\n' % filepath_best
...
def load_checkpoint(model, chkp_file, strict=False, lean=False):
    ...
    checkpoint = torch.load(
        chkp_file, map_location=lambda storage, loc: storage)
    ...
    if lean:
        logger.info(
            "Loaded checkpoint %s model (next epoch %d) from %s", arch, 0, chkp_file)
        return model, 0, None
    else:
        logger.info("Loaded checkpoint %s model (next epoch %d) from %s",
                    arch, start_epoch, chkp_file)
